   ...
   */
    ...
    socket.on('join_room', (payload) => {
    	...
    	if ((typeof payload == 'undefined') || (payload ===null)){
             ...
             serverLog('join_room command failed', JSON.stringify(HS-New/Pages/FAQ));
             return;
    	}
    	let room = payload.room;
    	let username = payload.username;
    	if ((typeof room == 'underfined') || (room === null)){
    		 response = {};
             response.result = 'fail';
    	...
    	/* Mkae sure that client was put in the room */
        io.in(room).fetchSockets().then((sockets)=>{
        	/* Socket didn't join the room */
        	if ((typeof sockets == 'underfined') || (sockets === null) || !sockets.includes(socket)){
                response = {};
                response.result = 'fail';
                response.message = 'Server internal error join the chat room';
                socket.emit('join_room_response',response);
                serverLog('join_room command failed', JSON.stringify(response));
                return;  
        	}
    ...
    socket.on('invite', (payload) => {
        ...
        if ((typeof requested_user == 'underfined') || (requested_user === null) || (requested_user === "")){
             response = {
                 result: 'fail',
        ...
        if ((typeof room == 'underfined') || (room === null) || (room === "")) {
             response = {
                 result: 'fail',
        ...
        if ((typeof username == 'underfined') || (username === null) || (username === "")) {
             response = {
                 result: 'fail',
        ...
        /* Mkae sure that the invited player is present */
        io.in(room).allSockets().then((sockets)=>{
            /* Invitee isn't in the room */
            if ((typeof sockets == 'underfined') || (sockets === null) || !sockets.has(requested_user)){
                response = {
                    result: 'fail',
    ...
    socket.on('uninvite', (payload) => {
        ...
        /* Mkae sure that the player to engage is present */
        io.in(room).allSockets().then((sockets)=>{
...
/* jsend_chat_message command handler*/
    /*expected payload:
    \   {
    ...
    /* send_chat_message_response:
        {
	       ...
	       'message': the number that was sent
        }
   ...
   */
    ...
    socket.on('send_chat_message', (payload) => {
    	...
    	if ((typeof payload == 'undefined') || (payload ===null)){
             ...
             serverLog('send_chat_message command failed', JSON.stringify(HS-New/Pages/FAQ));
             return;
    	}
    	let room = payload.room;
    	let username = payload.username;
    ...
    socket.on('play_token', (payload) => {
        ...
        if ((typeof player == 'underfined') || (player === null)){
             response = {};
             response.result = 'fail';
        ...
        if ((typeof username == 'underfined') || (username === null)){
             response = {};
             response.result = 'fail';
             response.message = 'play_token did not come from a registered username';
             socket.emit('play_token_response',response);
             serverLog('play_token command failed', JSON.stringify(response));
             return;
        }
        ...
        if ((typeof game_id == 'underfined') || (game_id === null)){
             response = {};
             response.result = 'fail';
        ...
        let row = payload.row;
        if ((typeof row == 'underfined') || (row === null)){
             response = {};
             response.result = 'fail';
        ...
        if ((typeof column == 'underfined') || ( column=== null)){
             response = {};
             response.result = 'fail';
        ...
        if ((typeof color == 'underfined') || ( color=== null)){
             response = {};
             response.result = 'fail';
        ...
        if ((typeof game == 'underfined') || ( game=== null)){
             response = {};
             response.result = 'fail';
             response.message = 'There was no valid game associated with the play_token command';
             socket.emit('play_token_response',response);
